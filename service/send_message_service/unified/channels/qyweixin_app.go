...
func (c *QyWeiXinAppChannel) SendUnified(msgObj interface{}, ins models.SendTasksIns, content *UnifiedMessageContent) (string, string) {
	...
	if contentType == FormatTypeText {
		if sendType == "group" {
			res, err = cli.SendGroupText(formattedContent, config.ChatID)
		} else {
			res, err = cli.SendMessageText(formattedContent, config.Touser, config.Toparty, config.Totag)
		}
		if err != nil {
		...
		if sendType == "group" {
			res, err = cli.SendGroupMarkdown(formattedContent, config.ChatID)
		} else {
			res, err = cli.SendMessageMarkdown(formattedContent, config.Touser, config.Toparty, config.Totag)
		}
		if err != nil {
			errMsg = fmt.Sprintf("发送失败：%s", err.Error())
		}
	} else {
		errMsg = fmt.Sprintf("未知的企业微信应用发送内容类型：%s", contentType)
	}
